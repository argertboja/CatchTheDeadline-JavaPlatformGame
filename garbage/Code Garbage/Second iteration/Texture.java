...
public class Texture 
{
  ...
  private BufferedImage playerRunSprite = null, playerRunSpriteM = null, playerJumpSprite = null, playerJumpSpriteM = null, 
    penSpin = null, eraserSpin = null, paintSpraySpin = null, examAnim = null, examAnimM = null, blankSheetFlip = null,homeworkAnim = null;
  ...
  public BufferedImage[] homeworkflipping = new BufferedImage[14];
  
  private final int width = 75, height = 127; // dimensions of sub image of player
  private SpriteSheet ps, psM, pj, pjM, penSprite, eraserSprite, paintSpraySprite, examSprite, examSpriteM, blankSheetSprite,homeworkSprite;
  ...
  public Texture() 
  {
    ...
    homeworkAnim=bufferedImageLoader.loadImg("/images/homeworkSprite.png"); 
    ...
    homeworkSprite= new SpriteSheet(homeworkAnim);
  ...
  public void generateTextures() 
  {
    int count = 0;
    for( int i = 1; i <= 7; i++ ) 
    { // divides the 'forward running sprite sheet of player' into sub-images
      for( int j = 1; j <= 6; j++ ) 
      {
    ...
    count = 0;
    for( int i = 1; i <= 7; i++ ) 
    { // divides the 'backward running sprite sheet of player' into sub-images
      for( int j = 1; j <= 6; j++ ) 
      { 
    ...
    count = 0;
    for( int i = 1; i <= 6; i++ ) 
    { // divides the 'forward jumping sprite sheet of player' into sub-images
      for( int j = 1; j <= 4; j++ ) 
      { 
    ...
    for( int i = 1; i <= 14; i++ ) 
    { 
      homeworkflipping[count] = homeworkSprite.divideImage( 1, i, 190, 190); // saving each sub-image as a single image onto the array
      count++;
      if( count >= 14 )
        break;
